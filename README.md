...
| `AddMissingOptions`       | Boolean    | `true`, `false`                           | When `true`, any **new** default keys missing from `curd.conf` are **appended once** (never a full rewrite every start). Set `false` to opt out of writing new keys. Defaults still apply in memory always. |
