...
| `AddMissingOptions`       | Boolean    | `true`, `false`                           | When `true`, missing options are appended to the config file on **curd version upgrades** (not every launch). Defaults still apply in memory always. |
| `VimKeys`                 | Boolean    | `true`, `false`                           | When `true`, selection menus use vim motions: `j`/`k`/`h`/`l` (and arrows) to move, `/` or `?` to search. Default: `false` (type-to-filter immediately). |
