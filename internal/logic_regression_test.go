...
func TestGetProviderNormalizesConfiguredProviderName(t *testing.T) {
	...
	if got := GetProvider().Name(); got != "animeworld" {
		t.Fatalf("expected animeworld provider when allanime is disabled, got %q", got)
	}
}
...
func TestGetProviderFallsBackWhenConfiguredProviderDisabled(t *testing.T) {
	...
	if got := GetProvider().Name(); got != "animeworld" {
		t.Fatalf("expected animeworld fallback for disabled provider, got %q", got)
	}
}
